...
def git_command(endpoint):
    #github_url=str(f"https://github.com/{endpoint.remote_user}/{endpoint.repo_name}")                      
    working_dir=str(f"{endpoint.local_dir}")
    #logger.debug(f"Github URL: {github_url}")
    logger.debug(f"Working Dir: {working_dir}")
    result = subprocess.run(str(f"cd {working_dir} && git fetch && git pull {endpoint.remote_name} {endpoint.remote_branch}"), capture_output = True, text = True, shell=True)
